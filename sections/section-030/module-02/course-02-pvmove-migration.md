...
> ```
>
> `vgdata` now spans three PVs (`#PV 3`); the spare shows a `VG` of `vgdata` with all its space free. `#LV` is still `1` — nothing about `applv`'s extent list changed. You only gave the pool somewhere to send data to.
