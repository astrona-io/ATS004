...
Already comfortable with an earlier part's material and just want to read a later one? Run `catchup part3` or `catchup part4` in the playground to skip straight to that part's starting state — see the playground's own overview doc, or run `catchup` with no arguments, for exactly what each one does before using it.
